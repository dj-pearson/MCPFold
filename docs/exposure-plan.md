...
The [token-query GEO plan](./token-query-geo-plan.md) covers being _cited_ for one query cluster. This
...
- **Broaden the top-of-funnel message.** "Save tokens" is being commoditized by native tool-search.
  ...
  CI/determinism). Token savings is the _proof_, not the whole pitch. See the homepage message test
...
posture (telemetry is off by default — see [telemetry.md](./telemetry.md)); measure the _web_ funnel,
...
numbers must be _collected_, not estimated, or the asset loses its credibility.
...
- Then render `/mcp-server-token-costs` (a sortable table) + wire it into the calculator presets so
  those become _real_ numbers, not the current honest estimates.
