...
test('footer link map resolves against the route table (no 404)', async ({ page }) => {
  ...
  const validInternal =
    /^\/($|install$|directory$|compare$|use-cases$|pricing$|features$|changelog$|security$|about$|privacy$|terms$|analytics$|accessibility$|brand$|glossary$|roadmap$|guides$|community$|blog$|mcp-token-calculator$|docs(\/|$))/;
