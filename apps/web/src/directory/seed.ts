...
/**
 * Curated MCP server directory (S7.4). A neutral, community-maintained list — mcpfold is not
 * affiliated with or endorsed by these projects or the MCP project. Entries prefill the editor
 * with a valid, ref-only server entry (tokens are `${provider:path}` placeholders, never values).
 *
 * This is the versioned seed; the DB-backed directory mirrors it (supabase/seed/directory.sql).
 * To add a server: append an entry with a unique `id`, a neutral one-line description, its
 * transport + launch (`command`/`args` for stdio, `url` for http/sse), an optional `tokenRef`
 * placeholder if it needs auth, and suggested tags. Keep copy factual — no endorsement language.
 */
